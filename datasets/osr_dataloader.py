...
class CIFAR100_Full:
    def __init__(self, dataroot='./data/cifar100', use_gpu=True, num_workers=8, batch_size=128, img_size=32):
        ...
        # === Train set ===
        trainset = CIFAR100(root=dataroot, train=True, download=True, transform=transform_train)
        self.train_loader = torch.utils.data.DataLoader(
            trainset, batch_size=batch_size, shuffle=True,
            num_workers=num_workers, pin_memory=pin_memory
        )
        ...
        # === Test set ===
        testset = CIFAR100(root=dataroot, train=False, download=True, transform=transform_test)
        self.test_loader = torch.utils.data.DataLoader(
            testset, batch_size=batch_size, shuffle=False,
            num_workers=num_workers, pin_memory=pin_memory
        )

        print(f"[CIFAR100_Full] Loaded: Train={len(trainset)}, Test={len(testset)}")